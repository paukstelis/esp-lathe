...
class ValueItem(CustomItem, CallbackItem):
    ...
    def draw(self):
        ...
        self.display.text(font, str.upper(self.name), x_pos,2, WHITE, BLACK)
        x_pos = self.display.width() - (len(str(self.value)) * 8) - 1
        self.display.text(font, str(self.value), x_pos, 20, WHITE, BLACK)

        self.display.hline(0, 12, self.display.width(), BLACK)
        #self.display.show()

    def select(self):
        return self.parent

    #switched up and down to avoid reversing encoder
    def up(self):
...
class Menu:
    ...
    def _item_line(self, item: MenuItem, pos):
        ...
        self.display.fill_rect(0, y, self.display.width(), self.line_height, WHITE)

        self.display.text(font, item.name, 0, y + v_padding,fg, bg)
        x_pos = self.display.width() - (len(item.get_decorator()) * self.font_width) - 1
        self.display.text(font, item.get_decorator(), x_pos, y + v_padding,fg, bg)

    def _menu_header(self, text):
        x = int((self.display.width() / 2) - (len(text) * self.font_width / 2))
        self.display.text(font, str.upper(self.current_screen.title), x, 3)
        self.display.hline(0, self.font_height + 2, self.display.width(), BLACK)
